        ...
        static void loadFromFile(string filename, bool useADLEyefinity, int delayInMs)
        {
            ...
            if (!string.IsNullOrWhiteSpace(json))
            {
                ...
                if (!profileAlreadyInUse(myDisplayConfig))
                {
                    ...
                    if (intelLibrary.IsInstalled)
                    {
                        ...
                        if (myDisplayConfig.IntelConfig.IsInUse)
                        {
                            ...
                            if (myDisplayConfig.IntelConfig.DisplayIdentifiers.Count > 0)
                            {
                                ...
                                {
                                    SharedLogger.logger.Trace($"VideoInfo/loadFromFile: The Intel display settings within {filename} are possible to use right now, so we'll use attempt to use them.");
                        ...
                        {
                            SharedLogger.logger.Trace($"VideoInfo/loadFromFile: Skipping applying Intel display settings as the NVIDIA settings are not in use in this display profile.");
                        }
                    ...
                    if ((amdLibrary.IsInstalled && itWorkedforAMD) || (nvidiaLibrary.IsInstalled && itWorkedforNVIDIA) || (intelLibrary.IsInstalled && itWorkedforIntel))
                    {
        ...
        static void equalFromFiles(string filename, string otherFilename)
        {
            ...
            if (!string.IsNullOrWhiteSpace(json)&&!string.IsNullOrWhiteSpace(otherJson))
            {
                ...
                if (displayConfig.WindowsConfig.Equals(otherDisplayConfig.WindowsConfig) && displayConfig.NVIDIAConfig.Equals(otherDisplayConfig.NVIDIAConfig) && 
                    displayConfig.AMDConfig.Equals(otherDisplayConfig.AMDConfig) && displayConfig.IntelConfig.Equals(otherDisplayConfig.IntelConfig))
                {
        ...
        static void equalFromFiles(string filename)
        {
            ...
            if (!string.IsNullOrWhiteSpace(json))
            {
                ...
                if (displayConfig.WindowsConfig.Equals(WinLibrary.GetLibrary().GetActiveConfig()) && 
                    ...
                    displayConfig.IntelConfig.Equals(IntelLibrary.GetLibrary().GetActiveConfig()))
                { 
